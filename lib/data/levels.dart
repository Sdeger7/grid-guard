...
class LevelCatalog {
  ...
  /// The single endless "base survival" map: no scripted waves, escalating raids
  /// generated by the game, and every free tile buildable so you can grow.
  static final LevelConfig survival = LevelConfig(
    ...
    gridCols: gridCols,
    gridRows: gridRows,
    path: _pathTemplate(1),
    safeZones: _survivalSafeZones(),
    waves: const [],
    startingMw: 0,
    coreIntegrity: 160,
  ...
  /// Every tile that isn't on the path or an endpoint is buildable in survival.
  static Set<TileCoord> _survivalSafeZones() {
    final path = _pathTemplate(1);
    final pathTiles = <TileCoord>{};
    for (var i = 0; i < path.length - 1; i++) {
      final a = path[i], b = path[i + 1];
      final dc = (b.col - a.col).sign, dr = (b.row - a.row).sign;
      var c = a.col, r = a.row;
      pathTiles.add(TileCoord(c, r));
    ...
    final spawn = path.first, core = path.last;
    final pads = <TileCoord>{};
    for (var r = 0; r < gridRows; r++) {
      for (var c = 0; c < gridCols; c++) {
        final t = TileCoord(c, r);
        if (pathTiles.contains(t) || t == spawn || t == core) continue;
        pads.add(t);
      }
    }
    return pads;
